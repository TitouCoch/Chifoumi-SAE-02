...
void ChifoumiPresentation::okClicked()
{
     ...
     {
         QSqlQuery query;
         query.prepare("INSERT INTO Resultat (Horodotage, NomJoueurHumain, ScoreJoueurHumain,ScoreJoueurMachine) "
         "VALUES (:Horodotage, :NomJoueurHumain, :ScoreJoueurHumain, :ScoreJoueurMachine)");
...
void ChifoumiPresentation::demandeResultat()
{
    //vérifie que l'on est bien dans l'état initial ou l'état pause
    if(getEtatJeu()==etatInitial)
    {
        //On change l'état du jeu sur état paramétrage
         setEtatJeu(etatResultat);
    }
    UnEtatJeu g = ChifoumiPresentation::getEtatJeu();
    switch (g) {
    //Etat résultat
    case etatResultat:{
        resultat * fenResultat = new resultat(nullptr);    //Création d'une intance de fenêtre de connexion
        fenResultat->majTableauResultat();
        fenResultat->show();
